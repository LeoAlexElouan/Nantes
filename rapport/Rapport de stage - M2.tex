...
Thierry Coquand published a few papers on the computational aspect of forcing \cite{10.5555/1839560.1839564}\cite{Mannaa_2014}. Liron Chohen and Vincent Rahli studied effectful, though undecidable, type theories based on sheaves \cite{07f7b3c6fe58463d80891f041156c6a4}\cite{Cohen_2024}.
...
\begin{itemize}
    ...
    \item Inductive equality types $\mathsf{eq}$ or $=$ with constructor $\mathsf{eq_refl}$
    \item Lists as $\mathsf{List}$, with predicate $\mathsf{In} : \Pi (A:\Type), A -> \mathsf{List}\ A -> \Type$ indicating when a element occurs in a list.
\end{itemize}
...
However, one the one hand, this type theory has equality reflection, proposititional extensionality, strict propositions and the axiom of unique choice.
...
We would like to note that these formulas define covers as they can be read as saying that some intersection of atoms can be covered by a family of intersection of atoms, and that they may be integrated in a deduction system as~: 
$$
...
%, and, notably, the inductive type with only $\ask$ can encode any.
...
This model highlights the importance of computations, since an inconsitent type theory can still be used to state a few thing as long as it computes properly.

\subsection{Presheaves}

The presheaf models was the most time consuming model, where neither universes nor dependant function types ended up being implemented, despite the time spent.
The main issue is the straightforward implementation is not strict \emph{i.e.} not all equalities are proven using $\mathsf{eq\_refl}$.
Furthermore, the proofs of these equalities are not immediate, and some proofs involve transport along non-trivial equalities.

The model uses functional extensionality and unicity of identity proofs as axioms, which helps with the above issues.

To give the broad strokes of the model, we use the following definition of a category, mostly to name the fields. It must be understood as a record (a fancy $\Sigma$), so the projections are named according to the fields.
$$ \textsf{Cat} := \left\{
    \begin{array}{rl}
        \mathsf{obj} :& \Type\\
...
$$
    ...
    \end{array}
$$
Notably, contexts are presheaves and not just types, and, strictly speaking, types are only presheaves in an empty context.
...
The model requires univalence and uses rewrite rules to simulate higher inductive types, which are absent in \rocq 9.0.0.

Types were interpreted as sheaves as described in definition \ref{IOshf}. Product types are relatively easy as an arbitrary product of sheaves is still a sheaf. Positives types, in my case boolean, where contructed as the higher inductive types with the same constructor as the original (true and false), together with $\ask$ and $\varepsilon$. The resulting type is equivalent to the sheafification of booleans but does not compute exactly the same.

One of the interest of type theory is that it yields meaningful computations from proof, so we were careful about that.
...
Together with the usual context, judgments depend on a \emph{forcing context} containing the finite approximation of $\alpha$, given by the following syntax, when $n$ and $b$ are a meta-theoretical integer and boolean respectively.
...
While in \ref{MBshtt} conversion rules could be added, it is impossible without specialising the system and deciding what $O_{i,\alpha}$ means.
...
The forcing contexts $\fctx$ are now finite subsets of $\Omega$ (subsets are lists accessed only through $\mathsf{In}$), and never cause ill-formation.
...
The above rule is both the expected congruence rule for conversion and a compatibility rule for typing that would be stated separately in a system with a pure typing judgement.
...
These last few rules allow for pursuing computations on $\digamma$.
...
A rather standard manner of studying the properties of a type theory is through logical relations.
We roughly follow the recipe of Abel \cite{10.1145/3158111}, which, since it is meant for a system closer to MLTT, would ease later scaling.
In pratice, we use LogRel \rocq as a reference point \cite{DBLP:conf/cpp/AdjedjLMPP24}.
We use \guillemotleft~logical relations~\guillemotright\ in a rather precise manner to designate this recipe.
Roughly speaking, we establish a reduction relation and interpret terms by their reduction to a normal form.

\subsection{System T extension}

To get a hang of $\digamma$ and logical relations we start by studying a extension of system T with $\digamma$
...
We then apply logical relations to our system T variant.
...
This work remains very incomplete.
I don't think attempting to prove normalisation of ShTT is the next step, same for any theories at the same level of generality.
It might be more interesting to either look at continuity for a more general $\alpha$ than in Martin Baillon's work, or find a simple enough theory with non-empty intersections.
The Zariski topos is a candidate for this approach, but the language extension is trickier.
Coquand has a few paper that we could revisit as well.
...
The \rocq\ code is accessible on~:
\url{https://github.com/LeoAlexElouan/Nantes/tree/main}

\section{Models}
The dialogue tree model is accessible at~:

\url{https://github.com/LeoAlexElouan/Nantes/blob/main/Mod%C3%A8les/CWFU_dialogue.v}

The exceptional model is accessible at~:

\url{https://github.com/LeoAlexElouan/Nantes/blob/main/Mod%C3%A8les/CWFU_exceptional.v}

The presheaf model is accessible at~:

\url{https://github.com/LeoAlexElouan/Nantes/blob/main/Mod%C3%A8les/CWFU_presheaf.v}

The presheaf model is accessible at~:

\url{https://github.com/LeoAlexElouan/Nantes/blob/main/Mod%C3%A8les/CWFU_sheaves.v}
\section{Logical relations}

The normalisation of modified system T using logical relations is accessible at~:

\url{https://github.com/LeoAlexElouan/Nantes/blob/main/MLTT/SystemTSheavesArity.v}
