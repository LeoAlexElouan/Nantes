...
The type theory devellopped by Martin Baillon, which we aim to generalise, proved continuity of all functional $(\nat -> \bool) -> \bool$.
...
Kripke models are models of propositional intuitionistic logic.
...
We note that by using the poset of contexts, Kripke models done with small categories are (only classicaly) complete for prpositional intuitionistic logic.

Sheaves themselves yield a more general kind of model, Beth models.

For every propositional variable $P$, $S_P$ must now be stable under union, and $U ||- \phi$ is now defined as follows~:
\begin{itemize}
    \item $U ||- P$ iff for all $V \leq U$ there exists $(V_i)_i$ with $V = \bigvee_i V_i$, such that for all $i$, $V_i \in S_P$
...
However, this type theory has equality reflection, proposititional extensionnality, strict propositions and the axiom of unique choice. This properties form a very strong, but also undecidable, system. In particular they are no known type theory with both unique choice and strict propositions which preserves canonicity and decidability of type-checking. They also have no universe in general, which, on the other hand, make them very weak.
...
Just like with topological sheaves, $j$-Sheaves form a topos. Note, however, that this definition of sheaves makes no reference to presheaves. They do still specialise to topological presheaves.
...
Now, the sheafified of a type doesn't exists in general, but if the theory admits quotient inductive types, it can then be defined as follow~:
...
$$ M, N ::= \dots | \digamma_i (M_\alpha)_{\alpha : A\ i}$$

When instanciating with finite $A i$, it would rather be  $\digamma_i M_1 \cdots M_n$.

The forcing contexts $\fctx$ are now subset of $\Omega$ (lists accessed only through wether they include some elements), and never cause ill-formation.
...
This theory is still incomplete. For example, to instantiate it with \ref{MBshtt}, even adding the necessary generic function $\alpha$, and setting $I := \nat$, $A i = \bool$ and $O_{n,b} := n\mapsto b$ so that \textsc{Dig}-$i$ may play the role of \textsc{Split}, we  have no way to provide the compatibility proof $n\mapsto \mtrue, n\mapsto \mfalse |- M_{\true} == M_{\false}$, which must be some form of ex-falso. \textsc{Ask}-$i$ cannot be used in stead of \textsc{Ask} either.

\section{Logical relations}\label{LogRel}

A rather standard manner of studying the property of a type theory is through following a recipe {\color{red} CITE GIRARD\,?}
