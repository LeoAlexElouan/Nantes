...
Inductive conv@{i} : ctx -> term@{i} -> term@{i} -> type@{i} -> Type@{i} :=
  ...
  | term_trans c A t u v : conv c t u A -> conv c u v A ->
    conv c t v A
  | digamma_i i c A k k' :
    (forall a a': Arity i, conv (ctx_cons_OO a a' c) (k a) (k' a') A) ->
    conv c (digamma i k) (digamma i k') A
  | digamma_erase i a c A k k' :
    List.In (O i a) (L c) -> (forall a a', conv (ctx_cons_OO a a' c) (k a) (k' a') A) ->
    conv c (digamma i k) (k' a) A
  | App_digamma i c A B k k' u u' :
    (forall a a', conv (ctx_cons_OO a a' c) (k a) (k' a') (A → B)) ->
    conv c u u' A -> conv c ((digamma i k) u) (digamma i (fun a => (k' a) u')) B
  | nat_r_digamma i c A k k' t0 t0' tS tS' :
    (forall a a', conv (ctx_cons_OO a a' c) (k a) (k' a') nat_typ) ->
    conv c t0 t0' A -> conv c tS tS' (nat_typ → A → A) ->
    conv c (nat_r t0 tS (digamma i k)) (digamma i (fun a => nat_r t0' tS' (k' a))) A
  | bot_r_digamma i c A k k' :
    (forall a a', conv (ctx_cons_OO a a' c) (k a) (k' a') bot_typ) ->
    conv c (bot_r (digamma i k)) (digamma i (fun a => bot_r (k' a))) A.
...
Inductive nat_conv_str : ctx -> term -> term -> Type :=
  | conv_nat_0 {c t t'} (r : Redstar t nat_0) (r' : Redstar t' nat_0) : nat_conv_str c t t'
  | conv_nat_S {c t t' u u'} (r : Redstar t (nat_S u)) (r' : Redstar t' (nat_S u'))
    (hu : nat_conv c u u') : nat_conv_str c t t'
  | conv_nat_Ne {c t t' n n'} (r : Redstar t n) (r' : Redstar t' n')
    (hneu : Ne n) (hneu' : Ne n') : nat_conv_str c t t'
  | conv_nat_digamma {i c t t' k k'} (r : Redstar t (digamma i k)) (r' : Redstar t' (digamma i k'))
    (hk : forall a a', nat_conv (ctx_cons_OO a a' c) (k a) (k' a')) : nat_conv_str c t t'
  with
    nat_conv : ctx -> term -> term -> Type :=
  | conv_nat_wkn {c t t'} : nat_conv_str c t t' -> nat_conv c t t'
  | conv_nat_digamma_left {i a c t t' k k'} (hin : List.In (O i a) (L c))
    (r : Redstar t (digamma i k)) (r' : Redstar t' (k' a))
    (hk : forall a a', nat_conv_str (ctx_cons_OO a a' c) (k a) (k' a')) : nat_conv c t t'
  | conv_nat_digamma_right {i a' c t t' k k'} (hin : List.In (O i a') (L c))
    (r : Redstar t (k a')) (r' : Redstar t' (digamma i k'))
    (hk : forall a a', nat_conv_str (ctx_cons_OO a a' c) (k a) (k' a')) : nat_conv c t t'.

Arguments conv_nat_0 {c t t'}.
Arguments conv_nat_S {c t t' u u'} r r' hu.
Arguments conv_nat_Ne {c t t' n n'} r r' hneu hneu'.
Arguments conv_nat_wkn {c t t'}.
...
Inductive bot_conv_str : ctx -> term -> term -> Type :=
  | conv_bot_Ne {c t t' n n'} (r : Redstar t n) (r' : Redstar t' n')
    (hneu : Ne n) (hneu' : Ne n') : bot_conv_str c t t'
  | conv_bot_digamma {i c t t' k k'} (r : Redstar t (digamma i k)) (r' : Redstar t' (digamma i k'))
    (hk : forall a a', bot_conv (ctx_cons_OO a a' c) (k a) (k' a')) : bot_conv_str c t t'
  with
    bot_conv : ctx -> term -> term -> Type :=
  | conv_bot_wkn {c t t'} : bot_conv_str c t t' -> bot_conv c t t'
  | conv_bot_digamma_left {i a c t t' k k'} (hin : List.In (O i a) (L c))
    (r : Redstar t (digamma i k)) (r' : Redstar t' (k' a))
    (hk : forall a a', bot_conv_str (ctx_cons_OO a a' c) (k a) (k' a')) : bot_conv c t t'
  | conv_bot_digamma_right {i a' c t t' k k'} (hin : List.In (O i a') (L c))
    (r : Redstar t (k a')) (r' : Redstar t' (digamma i k'))
    (hk : forall a a', bot_conv_str (ctx_cons_OO a a' c) (k a) (k' a')) : bot_conv c t t'.
...
Proof.
  ...
  + intros c t t' ht.
    ...
    - apply (conv_nat_digamma_left hin (star_comp r rs) (star_comp r' rs') (fun _ _ => iht _ _ _ _ star_nil star_nil)).
    - apply (conv_nat_digamma_right hin (star_comp r rs) (star_comp r' rs') (fun _ _ => iht _ _ _ _ star_nil star_nil)).
  + intros c t t' ht.
...
Proof.
  ...
  + revert c' hgeq.
    ...
    - eapply (conv_nat_digamma_left (L_geq hgeq _ hin) r r' (fun _ _ => (X _ _ _ (ctx_extend_OO hgeq)))).
    - eapply (conv_nat_digamma_right (L_geq hgeq _ hin) r r' (fun _ _ => (X _ _ _ (ctx_extend_OO hgeq)))).
  + revert c' hgeq.
    induction ht as [ | i c t t' k k' r r' hk ihk | | ]; intros c' hgeq.
...
Proof.
  ...
  - apply (conv_bot_digamma_left hin star_nil star_nil).
    change (sem_conv c (k a) (k' a) bot_typ).
...
Proof.
  ...
  + refine (antireduction _ (star_cons r_nat_digamma star_nil) star_nil).
    ...
    refine (fun _ _ => X _ _ (sem_conv_map ctx_tl_OO h0) (sem_conv_map ctx_tl_OO hS)).
...
(*   intros Γ t t' A ht.
  ...
  + refine (antireduction _ (star_cons r_bot_digamma star_nil) star_nil).
    ...
    refine (fun _ _ => (sem_conv_map ctx_tl_OO IHht)).
...
Proof.
  ...
  + induction ht as [ | | | i c t t' k k' r r' hk ihk | | ].
    ...
    - apply (conv_nat_digamma r' r).
      ...
      apply ihk.
    - apply (conv_nat_digamma_right hin r' r). clear a hin r'.
      intros a' a. change (sem_conv (ctx_cons_OO a' a c) (k' a') (k a) nat_typ).
      apply (sem_conv_map ctx_swap_OO (X _ _)).
    - apply (conv_nat_digamma_left hin r' r). clear a' hin r.
      intros a' a. change (sem_conv (ctx_cons_OO a' a c) (k' a') (k a) nat_typ).
      apply (sem_conv_map ctx_swap_OO (X _ _)).
  + induction ht as [ | i c t t' k k' r r' hk ihk | |].
...
Proof.
  ...
  - induction ht'; try inversion_Redstar.
    ...
    * apply (conv_nat_digamma_right hin star_nil r'0).
        ...
        apply (X _ _ (Redstar_Normal r' Normal_nat_0 r0) (ctx_trans hgeq ctx_tl_OO)).
  - induction ht'; try inversion_Redstar.
    * assert (nat_S u' = nat_S u0) by apply (Redstar_Normal_eq r' Normal_nat_S r0 Normal_nat_S).
      inversion H; subst.
      apply (conv_nat_S star_nil r'0 (IHht _ _ hgeq ht')).
    * apply (conv_nat_digamma_right hin star_nil r'0).
      intros a a''.
      refine (sem_conv_map (A:= nat_typ) (ctx_In_right hin) _).
      apply (X _ _ (Redstar_Normal r' Normal_nat_S r0) (ctx_trans hgeq ctx_tl_OO)).
  - induction ht'; try inversion_Redstar.
    * apply (conv_nat_Ne star_nil r'0 hneu hneu'0).
    * apply (conv_nat_digamma_right hin star_nil r'0).
      intros a a''.
      refine (sem_conv_map (A:= nat_typ) (ctx_In_right hin) _).
      apply (X _ _ (Redstar_Normal r' (Normal_Ne hneu') r0) (ctx_trans hgeq ctx_tl_OO)).
  - induction ht'; try inversion_Redstar.
    * assert (digamma i k' = digamma i0 k0) by apply (Redstar_Normal_eq r' Normal_digamma r0 Normal_digamma).
      inversion H; subst.
      ...
      * apply (conv_nat_digamma_right hin star_nil r'0).
        intros a a''.
        refine (sem_conv_map (A:= nat_typ) (ctx_In_right hin) _).
        apply (X0 _ _ (Redstar_Normal r' Normal_digamma r0) (ctx_trans hgeq ctx_tl_OO)).
  - induction ht'; try inversion_Redstar.
      1-5 : apply (conv_nat_digamma_left (L_geq hgeq _ hin) star_nil r'0).
      1-5 : intros a' a''.
      1-5 : apply (X _ _ _ _ (ctx_trans (ctx_left_In hin) (ctx_extend_OO hgeq))).
      * refine (conv_nat_0 _ star_nil).
        apply (Redstar_Normal r0 Normal_nat_0 r').
      * apply (sem_conv_map (A:=nat_typ) ctx_tl_OO). 
        refine (conv_nat_S _ star_nil ht').
        apply (Redstar_Normal r0 Normal_nat_S r').
      * refine (conv_nat_Ne _ star_nil hneu hneu').
        apply (Redstar_Normal r0 (Normal_Ne hneu) r').
      * apply (conv_nat_digamma (Redstar_Normal r0 Normal_digamma r') star_nil).
        intros a0 a'0.
        apply (sem_conv_map (A:= nat_typ) (ctx_extend_OO ctx_tl_OO)).
        apply hk0.
      * refine (conv_nat_digamma_left _ (Redstar_Normal r0 Normal_digamma r') star_nil _).
        apply (L_geq ctx_tl_OO _ hin0).
        intros a1 a'1.
        apply (sem_conv_map (A:= nat_typ) (ctx_extend_OO ctx_tl_OO)).
        apply hk0.
      * destruct (Redstar_noNormal r0 r').
        + apply (conv_nat_digamma_right hin0 star_nil r'0).
          intros a0 a'0.
          apply (sem_conv_map (A:=nat_typ) (ctx_In_right hin0)).
          apply (X0 _ _ r1 (ctx_trans hgeq ctx_tl_OO)).
        + apply (conv_nat_digamma_left (L_geq hgeq _ hin) star_nil r'0).
          intros a0 a'0.
          apply (X _ a).
          apply (ctx_trans (ctx_left_In hin) (ctx_extend_OO hgeq)).
          apply (conv_nat_digamma_right (L_geq ctx_tl_OO _ hin0) r1 star_nil).
          intros a1 a'1.
          apply (sem_conv_map (A:= nat_typ) (ctx_extend_OO ctx_tl_OO)).
          apply hk0.
  -  induction ht'; try inversion_Redstar.
      * assert (digamma i k' = digamma i0 k0) by apply (Redstar_Normal_eq r' Normal_digamma r0 Normal_digamma).
        ...
        apply (inj_right_pair (H:=eqdec)) in H2.
        destruct H2.
        apply (conv_nat_digamma_right (L_geq hgeq _ hin) star_nil r'0).
        intros a a''.
        apply (X _ a').
        apply (ctx_trans (ctx_left_In hin) (ctx_extend_OO hgeq)).
      ...
      * assert (digamma i k' = digamma i0 k0) by apply (Redstar_Normal_eq r' Normal_digamma r0 Normal_digamma).
        ...
        refine (antireduction (A:=nat_typ) _ star_nil r'0).
        apply (sem_conv_map (ctx_tl'_OO (L_geq hgeq _ hin) hin0)).
        apply hk0.
      * apply (conv_nat_digamma_right hin0 star_nil r'0).
        intros a a''.
        apply (sem_conv_map (A:=nat_typ) (ctx_In_right hin0)).
        apply (X0 _ _ (Redstar_Normal r' Normal_digamma r0)).
