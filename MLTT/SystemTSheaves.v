...
Record ctx : Type := ctx_pair { L : list I ; Γ : list type}.

Definition ctx_cons_i A c := ctx_pair (L c) (A::(Γ c)).
...
Inductive conv@{i} : ctx -> term@{i} -> term@{i} -> type@{i} -> Type@{i} :=
  ...
  | term_trans c A t u v : conv c t u A -> conv c u v A ->
    conv c t v A
  | digamma_i i L Γ A k k' : conv (ctx_pair (i::L) Γ) k k' A -> conv (ctx_pair L Γ) (digamma i k) (digamma i k') A
  | digamma_erase i c A k k' : conv c k k' A -> List.In i (L c) -> conv c (digamma i k) k' A
  | App_digamma i L Γ A B k k' u u' : conv (ctx_pair (i::L) Γ) k k' (A → B) -> conv (ctx_pair L Γ) u u' A -> conv (ctx_pair L Γ) ((digamma i k) u) (digamma i (k' u')) B
  | nat_r_digamma i L Γ A k k' t0 t0' tS tS' : conv (ctx_pair (i::L) Γ) k k' nat_typ -> conv (ctx_pair L Γ) t0 t0' A ->
    conv (ctx_pair L Γ) tS tS' (nat_typ → A → A) -> conv (ctx_pair L Γ) (nat_r t0 tS (digamma i k)) (digamma i (nat_r t0' tS' k')) A
  | bot_r_digamma i L Γ A k k' : conv (ctx_pair (i::L) Γ) k k' bot_typ -> conv (ctx_pair L Γ) (bot_r (digamma i k)) (digamma i (bot_r k')) A.
...
Inductive Red@{i} : term@{i} -> term@{i} -> Type@{i} :=
  ...
  | App_digamma_Red {i k u} : Red (digamma i k u) (digamma i (k u))
  | r_nat_digamma {i k t0 tS} : Red (nat_r t0 tS (digamma i k)) (digamma i (nat_r t0 tS k))
  | r_bot_digamma {i k} : Red (bot_r (digamma i k)) (digamma i (bot_r k)).
...
Inductive nat_conv : ctx -> term -> term -> Type :=
  ...
  | conv_nat_digamma {i c t t' k k'} (r : Redstar t (digamma i k)) (r' : Redstar t' (digamma i k'))
    (hk : nat_conv (ctx_pair (i::(L c)) (Γ c)) k k') : nat_conv c t t'
  | conv_nat_digamma_left {i c t t' k u'} (hin : List.In i (L c)) (r : Redstar t (digamma i k)) (r' : Redstar t' u') (hk : nat_conv c k u') : nat_conv c t t'
  | conv_nat_digamma_right {i c t t' u k'} (hin : List.In i (L c)) (r : Redstar t u) (r' : Redstar t' (digamma i k')) (hk : nat_conv c u k') : nat_conv c t t'.
...
Arguments conv_nat_digamma_left {i c t t' k u'} hin r r' hk.
Arguments conv_nat_digamma_right {i c t t' u k'} hin r r' hk.
...
Inductive bot_conv : ctx -> term -> term -> Type :=
  ...
  | conv_bot_digamma {i c t t' k k'} (r : Redstar t (digamma i k)) (r' : Redstar t' (digamma i k'))
    (hk : bot_conv (ctx_pair (i::(L c)) (Γ c)) k k') : bot_conv c t t'
  | conv_bot_digamma_left {i c t t' k u'} (hin : List.In i (L c)) (r : Redstar t (digamma i k)) (r' : Redstar t' u') (hk : bot_conv c k u') : bot_conv c t t'
  | conv_bot_digamma_right {i c t t' u k'} (hin : List.In i (L c)) (r : Redstar t u) (r' : Redstar t' (digamma i k')) (hk : bot_conv c u k') : bot_conv c t t'.

Arguments conv_bot_Ne {c t t' n n'} r r' hneu hneu'.
Arguments conv_bot_digamma {i c t t' k k'} r r' hk.
Arguments conv_bot_digamma_left {i c t t' k u'} hin r r' hk.
Arguments conv_bot_digamma_right {i c t t' u k'} hin r r' hk.
...
Proof.
  ...
  + intros c t t' ht.
    induction ht as
      [ c t t' rs rs' | c t t' v v' rs rs' ht _| c t t' n n' rs rs' | i c t t' k k' rs rs' hk _| i c t t' k v hin rs rs' ht iht | i c t t' v k' hin rs rs' ht iht].
    all: intros u u' r r'.
  ...
  + intros c t t' ht.
    induction ht as [ c t t' n n' rs rs'| i c t t' k k' rs rs' hk _ | i c t t' k v hin rs rs' ht iht | i c t t' v k' hin rs rs' ht iht].
    all: intros u u' r r'.
...
Lemma incl_cons_cons {i:I} {L L'} : List.incl L L' -> List.incl (i::L) (i::L').
Proof.
  intros hincl.
...
Lemma digamma_sound : forall {i c k k' A}, sem_conv (ctx_pair (i::(L c)) (Γ c)) k k' A ->
...
Proof.
  ...
  - intros c' hgeq u u' hu.
    ...
    apply hk.
    * apply (ctx_extend hgeq).
    * apply (sem_conv_map ctx_tl hu).
Defined.

Lemma digamma_left_sound : forall {i c k t' A}, List.In i (L c) -> sem_conv c k t' A -> sem_conv c (digamma i k) t' A.
Proof.
  intros i c k t' A. revert i c k t'.
  induction A.
  all: intros i c k t' hin hk.
  ...
  - intros c' hgeq u u' hu.
    ...
    apply (IHA2 _ _ _ _ (L_geq hgeq _ hin)).
    apply (sem_conv_map hgeq hk _ ctx_refl _ _ hu).
Defined.

Lemma digamma_right_sound : forall {i c t k' A}, List.In i (L c) -> sem_conv c t k' A -> sem_conv c t (digamma i k') A.
Proof.
  intros i c t k' A. revert i c t k'.
  induction A.
  all: intros i c t k' hin hk'.
  ...
  - revert t'' ht'.
    ...
    + induction ht'; try inversion_Redstar.
      * assert (digamma i k' = digamma i0 k0) by apply (Redstar_Normal_eq r' Normal_digamma r Normal_digamma).
        inversion H; subst.
        apply (conv_nat_digamma star_nil r'0 (IHht _ ht')).
      * assert (digamma i k' = digamma i0 k0) by apply (Redstar_Normal_eq r' Normal_digamma r Normal_digamma).
        inversion H; subst.
        apply (conv_nat_digamma_left hin star_nil r'0).
        change (sem_conv c k u' nat_typ).
        apply (sem_conv_map (ctx_tl' hin)).
        change (sem_conv c k0 u' nat_typ) in ht'.
    ...
    + induction ht'; try inversion_Redstar.
      ...
      * refine (conv_nat_Ne _ star_nil hneu hneu').
        apply (Redstar_Normal r (Normal_Ne hneu) r').
      * refine (conv_nat_digamma _ star_nil ht').
        apply (Redstar_Normal r Normal_digamma r').
    ...
    + induction ht'; try inversion_Redstar.
      ...
      * assert (digamma i k' = digamma i0 k) by apply (Redstar_Normal_eq r' Normal_digamma r Normal_digamma).
        inversion H; subst.
        apply IHht.
        apply (antireduction (ht': sem_conv _ _ _ nat_typ) star_nil r'0).
      * apply (conv_nat_digamma_right hin0 star_nil r'0).
